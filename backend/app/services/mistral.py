...
Audio transcription uses Groq's Whisper API (whisper-large-v3-turbo).
"""
import base64
import json
...
async def analyze_text_threat(text: str) -> dict:
    ...
    messages = [{
        ...
        "content": (
            ...
            "DO NOT invent threats that are not present. "
            "Even when safe, still fill these fields with a safety explanation in Spanish:\n"
            "- manipulation_summary: explain why this content appears safe and what normal indicators are present "
            "(e.g. 'El contenido analizado corresponde a una imagen ordinaria sin texto de alerta, "
            "sin dominios sospechosos, sin numeros de telefono no verificados y sin tacticas de presion psicologica detectadas.')\n"
            "- recommended_actions: list 2-3 short positive confirmations of safety indicators verified, e.g. "
            "'No se detectaron dominios o URLs sospechosas', "
            "'No hay tacticas de urgencia o coercion presentes', "
            "'El contenido no impersonifica ninguna institucion'\n\n"
...
async def transcribe_audio(audio_bytes: bytes, mime_type: str = "audio/ogg") -> str:
    ...
    if not settings.groq_api_key:
        return "[Transcription skipped] Set GROQ_API_KEY to enable audio analysis."
    ...
    try:
        ...
        print(f"[transcribe_audio] status={resp.status_code} body={resp.text[:300]!r}")

        if resp.status_code != 200:
            raise Exception(f"Groq STT {resp.status_code}: {resp.text}")
